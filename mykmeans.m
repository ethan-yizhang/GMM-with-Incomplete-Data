...
pnames = {   'distance'  'start' 'replicates' 'emptyaction' 'onlinephase' 'options' 'maxiter' 'display' 'rdseed'};
dflts =  {'sqeuclidean' 'plus'          []  'singleton'         'off'        []        []        []     []};
[distance,start,reps,emptyact,online,options,maxit,display,rdseed] ...
...
if hadNaNs
    ...
    function cellout = loopBody(rep,S)
        ...
        S = RandStream('mlfg6331_64','seed',rdseed);
        rdseed = rdseed + randi(S,1000000,1);
        S = RandStream('mlfg6331_64','seed',rdseed);
        rdseed = rdseed + randi(S,1000000,1);
        S = RandStream('mlfg6331_64','seed',rdseed);
        if display > 1 % 'iter'
